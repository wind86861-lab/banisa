...
import {
    ...
    CheckCircle2, XCircle, Power, Loader2,
    Lock, Sparkles,
            ...
            <div className="pay-hero__row">
                <div>
                    <div className="pay-hero__title">
                        <ShieldCheck size={22} /> CLICK — to'lov tizimi
                    </div>
                    ...
                    <div className="pay-spark">
                        <ResponsiveContainer>
                            <AreaChart data={data} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
                                <defs>
                                    <linearGradient id="clickOkGrad" x1="0" y1="0" x2="0" y2="1">
                                        <stop offset="0%" stopColor="#0078d4" stopOpacity={0.35} />
                                        <stop offset="100%" stopColor="#0078d4" stopOpacity={0} />
                                    </linearGradient>
                                </defs>
                                <Area type="monotone" dataKey="total" stroke="#0078d4" strokeWidth={2} fill="url(#clickOkGrad)" />
                                <Tooltip
                                    contentStyle={{
                                        background: '#fff',
...
function KeyVaultCard({ config, onRotate }) {
    ...
    return (
        <div className="pay-card">
            ...
            <div className="pay-key-row">
                <div>
                    <div className="pay-key-row__label">Service ID</div>
                    <div className="pay-key-row__value">{config.serviceId}</div>
                </div>
            </div>
            ...
            {config.merchantUserId && (
                <div className="pay-key-row">
                    <div>
                        <div className="pay-key-row__label">Merchant User ID</div>
                        <div className="pay-key-row__value">{config.merchantUserId}</div>
                    </div>
                </div>
            )}
            ...
            <div className="pay-key-row">
                <div>
                    <div className="pay-key-row__label">Production secret_key</div>
            ...
            {config.hasTestKey && (
                <div className="pay-key-row">
                    <div>
                        <div className="pay-key-row__label">Test secret_key</div>
            ...
            <div className="pay-url-block">
                ...
                <div className="pay-qr-hint" style={{ padding: '12px 14px', background: 'var(--bg, #f8fafc)', borderRadius: 12, border: '1px dashed var(--border-color, #e2e8f0)' }}>
                    <div style={{ marginBottom: 4 }}>
                        <strong>CLICK kabinetiga joylash:</strong>
                    </div>
                    Yuqoridagi URL'ni CLICK kabinetiga <strong>Prepare URL</strong> VA
        ...
        <div className="pay-card">
            ...
            {(!items || items.length === 0) ? (
                <div className="pay-empty">
                    ...
                    CLICK kabinetiga URL'ni joylab, test tranzaksiya yuboring.
                </div>
            ) : (
                <div className="pay-log">
                    {items.map((it) => {
                        const ok = it.errorCode === null || it.errorCode === 0;
                        return (
                            <div key={it.id} className={`pay-log__item ${ok ? '' : 'pay-log__item--err'}`}>
                                ...
                                <div style={{ minWidth: 0 }}>
                                    ...
                                    {(it.orderId || it.clickTransId) && (
                                        <div className="pay-log__order">
                                            {it.orderId && <span>Order: {it.orderId.slice(0, 12)}…</span>}
                                            {it.clickTransId && <span style={{ marginLeft: 8 }}>CLICK: {it.clickTransId}</span>}
                                            {it.isTestMode && <span style={{ marginLeft: 8, color: '#b45309' }}>TEST</span>}
                                        </div>
                                    )}
                                    {!ok && (it.errorMsg || it.errorCode !== null) && (
                                        <ClickTroubleshooter errorCode={it.errorCode} errorMsg={it.errorMsg} />
                                    )}
                                </div>
...
export default function ClickTab() {
    ...
    const { data: configData, isLoading: configLoading } = useQuery({
        queryKey: ['clinic', 'click', 'config'],
        queryFn: async () => (await api.get('/clinic/payments/click/config')).data?.data,
    });
    ...
    const { data: stats, isLoading: statsLoading } = useQuery({
        queryKey: ['clinic', 'click', 'stats'],
        queryFn: async () => (await api.get('/clinic/payments/click/stats?range=24h')).data?.data,
        enabled: !!config,
        refetchInterval: 30_000,
    });

    const { data: recent, isLoading: recentLoading } = useQuery({
        queryKey: ['clinic', 'click', 'recent'],
        queryFn: async () => (await api.get('/clinic/payments/click/recent?limit=15')).data?.data?.items ?? [],
        enabled: !!config,
        refetchInterval: 30_000,
    });
    ...
    const toggleMode = useMutation({
        mutationFn: async (next) =>
            (await api.patch('/clinic/payments/click/config/mode', { isTestMode: next })).data,
        onSuccess: () => {
